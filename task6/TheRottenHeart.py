...
def talk(ally: objects.Ally, player: objects.Player):
    ...
    if ally == neptune:
        ...
        if ally.get_reputation() == 0:
            ...
            print("[Гігея]: Нічого. Нам не вистачає інгрідієнтів.\n")
...
def print_win():
    ...
    print("перетворюється в купи пилу та попелу. До тебе ззаду золота пані, та вручає тобі кошичок з купою карамельних")
...
Тут простягається головна алея Льова, на якій в даличині видно театр опери та балету.
...
У світлій залі попід стін стоять шафи з величезною кількістю різних пляшечок та посудиню
...
Усюди чорний міцелій, товщина якого інколи більша ніж обхавт людської руки.
